...
namespace engine
{
	...
	void VulkanRenderer::pRecreateSwapChain() {
		...
		for (auto i = 0; i < m_pVkData.defaultRenderPass.renderPipelines.size(); i++)
			vk::createGraphicsPipeline(m_pVkData, m_pVkData.defaultRenderPass, m_pVkData.defaultRenderPass.renderPipelines[i], m_pVertexInfo, m_pResourceManagerRef->m_pDefaultRenderPassInfo);
		...
		for (auto& pass : m_pResourceManagerRef->renderPassPool)
		{
			...
			for (auto i = 0; i < pass.renderPipelines.size(); i++)
				vk::createGraphicsPipeline(m_pVkData, pass, pass.renderPipelines[i], m_pVertexInfo, m_pResourceManagerRef->m_pRenderPassInfo[pass.id]);
