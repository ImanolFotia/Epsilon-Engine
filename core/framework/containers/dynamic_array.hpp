...
template <typename T> class DynamicArray {
  template <typename iterator_type = T> struct DynamicArrayIterator {
    using iterator_category = std::random_access_iterator_tag;
    using difference_type = std::ptrdiff_t;
    using value_type = iterator_type;
    using pointer = iterator_type *; // or also value_type*
    using reference = iterator_type &;
    using const_reference = const iterator_type &;

    DynamicArrayIterator(pointer ptr = nullptr) : m_pPtr(ptr) {}
    DynamicArrayIterator(const DynamicArrayIterator<iterator_type> &) = default;

    ~DynamicArrayIterator() {}

    DynamicArrayIterator<iterator_type> &operator=(const DynamicArrayIterator<iterator_type> &rawIterator) = default;
    DynamicArrayIterator<iterator_type> &operator=(T *ptr) {
      m_pPtr = ptr;
      return (*this);
    }

    reference operator*() { return *m_pPtr; }
    const_reference operator*() const { return *m_pPtr; }
    pointer operator->() { return m_pPtr; }

    // Prefix increment
    DynamicArrayIterator<iterator_type> &operator++() {
      m_pPtr++;
      return *this;
    }

    // Postfix increment
    DynamicArrayIterator<iterator_type> operator++(int) {
      DynamicArrayIterator<iterator_type> tmp = *this;
      ++(*this);
      return tmp;
    }
    ...
    DynamicArrayIterator<iterator_type> operator-(const difference_type &val) {
      m_pPtr = m_pPtr - val;
      return *this;
    }

    difference_type operator-(const DynamicArrayIterator<iterator_type> &rawIterator) { 
        return std::distance(rawIterator.m_pPtr, this->m_pPtr); 
    }
    ...
    friend bool operator==(const DynamicArrayIterator<iterator_type> &a, const DynamicArrayIterator<iterator_type> &b) { return a.m_pPtr == b.m_pPtr; };
    friend bool operator!=(const DynamicArrayIterator<iterator_type> &a, const DynamicArrayIterator<iterator_type> &b) { return a.m_pPtr != b.m_pPtr; };
    pointer m_pPtr;
  };
...
private:
  ...
  void change_capacity(size_type new_capacity) {
    ...
    size_type working_capacity = new_capacity > old_capacity ? old_capacity : new_capacity;
...
public:
  ...
  constexpr const_iterator cbegin() const noexcept { return const_iterator(m_pData); }

  constexpr const_iterator cend() const noexcept { return const_iterator(m_pData + m_pSize); }
  ...
  */
  template <class... Args> constexpr reference_type emplace(const_reference pos, Args &&...args) { throw std::runtime_error("emplace not implemented"); }

  template <class... Args> constexpr reference_type emplace_back(Args &&...args) {
    if (m_pSize >= m_pCapacity) {
  ...
  constexpr const_iterator erase(const_iterator pos) {
    assert(pos != cend() && "iterator is end");

    difference_type ps = pos - cbegin();
    const_iterator p = cbegin() + ps;

    for (const_iterator it = p; p != cend();) {
      (it++) = (++p);
    }
    m_pSize--;
    change_capacity(m_pSize);
    ...
    if (pos != cend())
      return const_iterator(cbegin() + ps - 1);
