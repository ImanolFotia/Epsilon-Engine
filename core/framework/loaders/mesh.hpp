...
namespace framework
{
    ...
    public:
        ...
        Mesh(std::vector<EML::t_Vertex> inVertices, std::vector<uint32_t> indices, MeshMaterial material, size_t vOffset, size_t iOffset)
        {
            ...
            auto Pos2Vec3 = [](EML::t_Position vtx) {
                return glm::vec3(vtx.x, vtx.y, vtx.z);
            };

            auto Tan2Vec3 = [](EML::t_Tangent vtx) {
                return glm::vec3(vtx.x, vtx.y, vtx.z);
            };

            auto Nor2Vec3 = [](EML::t_Normal vtx) {
                return glm::vec3(vtx.x, vtx.y, vtx.z);
            };

            auto Bi2Vec3 = [](EML::t_Bitangent vtx) {
                return glm::vec3(vtx.x, vtx.y, vtx.z);
            };

            auto Coord2Vec2 = [](EML::t_TexCoord vtx) {
                return glm::vec2(vtx.s, vtx.t);
            };


            for (auto& vtx : inVertices) {
                m_pData.mesh.Vertices.emplace_back(Pos2Vec3(vtx.position), Coord2Vec2(vtx.texcoord), Nor2Vec3(vtx.normal), glm::vec4(1.0), Tan2Vec3(vtx.tangent), Bi2Vec3(vtx.bitangent));
            }
            ...
            m_pData = { 
                .numVertices = m_pData.mesh.Vertices.size(),
        ...
        void setMinMaxPoints(auto minmax) {
            m_pMinMaxPoints = minmax;
        }

        auto getMinMaxPoints() {
            return m_pMinMaxPoints;
        }

        void setRelativePosition(auto position) {
            m_pRelativePosition = position;
        }
