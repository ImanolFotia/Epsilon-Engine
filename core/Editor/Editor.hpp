...
namespace Epsilon::Editor::GUI
{
    ...
    static void Init(std::shared_ptr<Platform::WindowBase> window)
    {
        ...
        KeyBoard::KeyboardEventHandler.addListener(&onKeyPress);
    ...
    static void SparkLine(const char *id, const char *overlay, const float *values, int count, float min_v, float max_v, int offset, const ImVec4 &col, const ImVec2 &size)
    {
    ...
    static void MainWindow(std::vector<std::shared_ptr<EntityBase>> &EntityList)
    {
        ...
        if (show_new_project_window)
        {
            ...
            Project::NewProject(&show_new_project_window);
        }
