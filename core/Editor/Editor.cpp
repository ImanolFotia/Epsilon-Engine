...
namespace Epsilon::Editor
{
    void Editor::Init(std::shared_ptr<Platform::WindowBase> window)
    {
        ...
        Mouse::MouseEventHandler += beacon::bind(&onLeftClickCallback, this);
        
        Mouse::MouseEventHandler += beacon::bind(&onMouseWheelCallback, this);

        KeyBoard::KeyboardEventHandler += beacon::bind(&onKeyPress, this);
    ...
    void Editor::SystemStatus()
    {
        ...
        ImGui::BulletText("Total Physical Memory: %lluMB", mMemory.TotalPhysicalMemory() / div);
        ImGui::BulletText("Total Virtual Memory: %lluMB", mMemory.TotalVirtualMemory() / div);
        ImGui::BulletText("Physical Memory in use: %lluMB", physical_mem_in_use / div);
        ImGui::BulletText("Virtual Memory in use: %lluMB", mMemory.CurrentVirtualUsage() / div);
        ImGui::BulletText("Memory used by this process: %lluMB", mMemory.CurrentPhysicalUsageByCurrentProcess() / div);
                ...
                if (sSelectedEntity->HasRenderComponent())
                {
                    ...
                    if (ImGui::CollapsingHeader("Albedo"))
                    {
                        ...
                        if (sUseAlbedoTexture)
                        {
                            ...
                            if (ImGui::ImageButton((void *)(unsigned int)textureId, ImVec2(64, 64), ImVec2(0, 0), ImVec2(1, 1), 8, ImVec4(1.0f, 1.0f, 1.0f, 1.0f), ImVec4(1.0f, 1.0f, 1.0f, 1.0f)))
                            {
...
*/
        ...
        void Editor::TextureList(std::pair<std::string, std::shared_ptr<Renderer::Texture2D>> *refTexture)
        {
            ...
            for (auto &texture : TextureArray)
            {
                ...
                if (ImGui::ImageButton((void *)(unsigned int)textureId, ImVec2(imgSize, imgSize), ImVec2(0, 0), ImVec2(1, 1), -1, ImVec4(1.0f, 1.0f, 1.0f, 1.0f), ImVec4(1.0f, 1.0f, 1.0f, 0.5f)))
                {
                    *refTexture = texture;
