...
namespace engine::audio {
	...
	void ALAudioManager::setSourceState(Ref<AudioSource> source_ref, AudioState state) {
		...
		if (state == AudioState::PLAYING) al::playSource(*source);
		if (state == AudioState::PAUSED) al::pauseSource(*source);
		if (state == AudioState::STOPPED) al::stopSource(*source);
	}

	AudioState ALAudioManager::getSourceState(Ref<AudioSource> source_ref) {
		auto source = *sourcesPool.get(source_ref);
		auto state = al::getSourceState(source);
		if (state == AL_PLAYING) return AudioState::PLAYING;
		if (state == AL_STOPPED) return AudioState::STOPPED;
		if (state == AL_PAUSED) return AudioState::PAUSED;
	}
