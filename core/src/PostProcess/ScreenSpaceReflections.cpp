...
{
    ScreenSpaceReflections::ScreenSpaceReflections(int w, int h) : Effect(w, h) {
        m_Width = w;
        m_Height = h;
        m_pShader = (std::shared_ptr<Shader>) new Shader("shaders/SSR.vglsl", "shaders/SSR.fglsl");
    }

    GLuint ScreenSpaceReflections::Apply(GLuint gFinalImage, GLuint gPosition, GLuint gNormal, GLuint gExpensiveNormal, GLuint gExtraComponents,
    ...
    void ScreenSpaceReflections::PrepareSSRTexture()
    {
