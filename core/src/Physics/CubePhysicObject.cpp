...
namespace Physics {
    btRigidBody_ptr CubePhysicObject::addObject(glm::vec3  Position, float  Mass, MIN_MAX_POINTS minmaxpoints, float scale) {
		btTransform t;
		t.setIdentity();
		btVector3 pos = btVector3(Position.x, Position.y, Position.z);
		t.setOrigin(pos);
		CubeShape = (std::shared_ptr<btBoxShape>) new btBoxShape(btVector3(
                                                                     scale * (glm::length(minmaxpoints.MAX_X - minmaxpoints.MIN_X)/2),
                                                                     scale * (glm::length(minmaxpoints.MAX_Y - minmaxpoints.MIN_Y)/2),
                                                                     scale * (glm::length(minmaxpoints.MAX_Z - minmaxpoints.MIN_Z)/2))
                                                                );
