...
namespace Epsilon
{
    ...
    Epsilon::Epsilon(const char *name) : App(name)
    {
        ...
        editor.CameraMode = &this->m_CameraMode;
        // window->HideCursor();
        ...
        mCameraData = std::make_shared<API::OpenGL::UniformBuffer>(sizeof(GlobalTransform_t), 1, 0, GL_DYNAMIC_DRAW);
        mCameraData->Bind();
    ...
    void Epsilon::onCreate()
    {
        ...
        m_StrategyController = std::make_shared<Renderer::StrategyController>(glm::vec3(0.0f), glm::vec3(-0.5f, 0.0f, -0.5f), 0.5f, 0.5f);
        editor.m_StrategyControllerReference = m_StrategyController;
        ...
        ref.addTextureToQueue("textures/epsilon/futuristic-panels1-albedo.png");
        ref.addTextureToQueue("textures/epsilon/futuristic-panels1-normal-dx.png");
        ref.addTextureToQueue("textures/epsilon/futuristic-panels1-metallic.png");
        ref.addTextureToQueue("textures/epsilon/futuristic-panels1-roughness.png");

        std::cout << "Material Id: " << SphereMaterial << std::endl;
        std::cout << "Material is nullptr: " << (mat != nullptr ? "no" : "yes") << std::endl;
        ref.ForceLoading();

        mat->setMaterial(Renderer::MaterialBase::MaterialParameter::Albedo, ref.Get<Tex2D_ptr>("textures/epsilon/futuristic-panels1-albedo.png"));
        mat->setMaterial(Renderer::MaterialBase::MaterialParameter::Metallic, ref.Get<Tex2D_ptr>("textures/epsilon/futuristic-panels1-metallic.png"));
        mat->setMaterial(Renderer::MaterialBase::MaterialParameter::Normal, ref.Get<Tex2D_ptr>("textures/epsilon/futuristic-panels1-normal-dx.png"));
        mat->setMaterial(Renderer::MaterialBase::MaterialParameter::Roughness, ref.Get<Tex2D_ptr>("textures/epsilon/futuristic-panels1-roughness.png"));
        /*
        ...
        */
...

        {
            ...
            glm::vec3 tScale = glm::vec3(1.0);
            glm::quat tRotation = glm::quat(1.0, 0.0, 0.0, 0.0);
            std::string tModelName = "models/vodka.eml";

            std::shared_ptr<EntityBase> _Entity = std::make_shared<EntityBase>(tPosition, tScale, tRotation);

            // Component::RenderComponent_ptr _RComp = std::make_shared<Component::RenderComponent>(mCube, "Transmission");
            auto _RComp = std::make_shared<Component::RenderComponent>(tModelName, tPosition, "Transmission");
            _RComp->setShader("Transmission");
            _RComp->CastsShadows(false);
            _RComp->setTransparency(false);
            _RComp->setVisibility(true);
            _RComp->setRenderId(EntityList.size());
            _RComp->isDoubleFaced(false);
            ...
            TransparentCubeId = EntityList.size();
...
*/
    ...
    RenderSplashScreen("Loading Textures...");*/
        ...
        // auto t = Engine::Get().getGlobalTransform();
        mCameraData->Init(&mGlobalTransform);
        sun->Update();

        {
            std::vector<Transform> tTransformations;
            ...
            for (int a = 0; a < 7; a++)
                for (int b = 0; b < 7; b++)
                    for (int c = 0; c < 7; c++)
                    {
                        ...
                        Transform t(positions, tScale, tRotation);
                        tTransformations.push_back(t);
                    }

            std::shared_ptr<Renderer::Sphere> sphere_probe = std::make_shared<Renderer::Sphere>(20);
            mInstancedDrawable = std::make_shared<Renderer::InstancedDrawable>(sphere_probe, tTransformations);
        }
...
*/
        ...
        for (unsigned int i = 0; i < EntityList.size(); ++i)
        {
            ...
            if (i == TransparentCubeId)
            {
                EntityList[i]->setShader("Transmission");
                continue;
            }
...
*/
    ...
    void Epsilon::LoadShaders(void)
    {
        ...
        Shaders["AmbientProbe"] = std::make_shared<Shader>("shaders/AmbientProbe.vert.glsl", "shaders/AmbientProbe.frag.glsl");
        ...
        ResourceManager::Get().requestShader("shaders/Transmission.vglsl", "shaders/Transmission.glsl", "Transmission");
    ...
    void Epsilon::LoadGeometry(void)
    {
        ...
        terrain = std::make_shared<Terrain>("materials/terrain_1024_alpine3_height.png",
                                            ...
                                            "Rock_6_n.png",
                                            "Rock_6_s.png",
                                            "Rock_6_ao.png", 2.0, 1000, glm::vec3(0, -67.0, 0));/*
*/
        for (int i = 0; i < 20000; i++)
        {
            ...
            int chance = rand() % 2 + 1;
            if (chance == 1)
                grassPos.push_back(glm::vec3((rand() % 30) - 15.0, 0.5, (rand() % 30) - 15.0));
            else if (chance == 2)
                grasspos2.push_back(glm::vec3((rand() % 30) - 10.0, 0.5, (rand() % 30) - 15.0));
        }
    ...
    */
    ...
    void Epsilon::Render3D(std::shared_ptr<Shader> shader)
    {
        ...
        this->SetUniforms(Shaders["Terrain"], glm::vec3(0, 0, 0), glm::vec3(1.0), glm::quat(0, 0, 0, 0));
         terrain->RenderTerrain(Shaders["Terrain"].get());
        glCullFace(GL_BACK);
    ...
    void Epsilon::RenderShadows(std::shared_ptr<Shader> shader)
    {
        ...
        this->SetUniforms(shader, glm::vec3(0, 0, 0), glm::vec3(1.0), glm::quat(0, 0, 0, 0));
         terrain->RenderTerrain(shader.get());
    ...
    void Epsilon::SetUniforms(std::shared_ptr<Shader> shader, glm::vec3 position, glm::vec3 scale, glm::quat rotation)
    {
        ...
        shader->PushUniform("FrameNumber", FrameCount);
        shader->PushUniform("TAAJitter", TAAJitter);
    ...
    void Epsilon::ComputeCamera(CAMERA_MODE mode, glm::vec3 position, glm::vec3 direction, glm::mat4 proj, glm::mat4 view)
    {
        ...
        else
        {
            ...
            this->eCamera[mCurrentCamera]->setPosition(m_StrategyController->getPosition());
            this->eCamera[mCurrentCamera]->setDirection(m_StrategyController->getDirection());

        if (PP)
        {

            eCamera[mCurrentCamera]->jitter_multiplier = PP->getPostProcessData().AntiAliasingSettings.JitterMultiplier;
            TAAJitter = this->eCamera[mCurrentCamera]->UpdateMatrices(mWindow->FrameNumber(), WIDTH, HEIGHT, PP->getPostProcessData().AntiAliasingSettings.Active);
        }
        else
            TAAJitter = this->eCamera[mCurrentCamera]->UpdateMatrices(mWindow->FrameNumber(), WIDTH, HEIGHT);

        auto &EngineRef = Engine::Get();

        GlobalTransform_t t = {};

        t.direction = glm::vec4(eCamera[mCurrentCamera]->getDirection(), 1.0f); // glm::vec4(Orientation, 1.0f);
        t.position = glm::vec4(eCamera[mCurrentCamera]->getPosition(), 1.0f);   // glm::vec4(Position, 1.0f);
        t.view = eCamera[mCurrentCamera]->getViewMatrix();
        t.projection = eCamera[mCurrentCamera]->getProjectionMatrix();
        t.invview = glm::inverse(t.view);
        t.invprojection = glm::inverse(t.projection);
        t.near = eCamera[mCurrentCamera]->getNearPlane();
        t.far = eCamera[mCurrentCamera]->getFarPlane();

        t.CurrentSelectedEntity = editor.sSelectedEntityIndex;

        mCameraData->Update(sizeof(GlobalTransform_t), 0, (const void *)&t);
        mCameraData->Bind();
    }
...
*/
        ...
        auto tShader = ResourceManager::Get().useShader("Transmission");
        tShader->Use();

        glActiveTexture(GL_TEXTURE5);
        glBindTexture(GL_TEXTURE_2D, PP->mFramebufferBlur[0]->getRenderTargetHandler(0));
        tShader->PushUniform("blurredTexture", 5);

        auto &Instance = ResourceManager::Get();
        int CubemapId = -1;

        if (Instance.cubemapsLoaded && CubemapId == -1)
        {
            CubemapId = Instance.NearestCubeMap(EntityList[TransparentCubeId]->getPosition());
        }

        auto cubemap = Instance.getCubemap(CubemapId);
        ...
        tShader->PushUniform("roughness", 0.5f);
        tShader->PushUniform("resX", PP->width);
        tShader->PushUniform("resY", PP->height);
        this->SetUniforms(tShader, EntityList[TransparentCubeId]->getPosition(), EntityList[TransparentCubeId]->getScale(), EntityList[TransparentCubeId]->getRotation());
        // mCube->Render();
        {
            glm::mat4 Model = glm::mat4(1.0);
            glm::mat4 ScaleMatrix = glm::scale(glm::mat4(1.0), EntityList[TransparentCubeId]->getScale());
            glm::mat4 TranslationMatrix = glm::translate(glm::mat4(1.0), EntityList[TransparentCubeId]->getPosition());
            glm::mat4 RotationMatrix;
            RotationMatrix = glm::mat4(1) * glm::toMat4(glm::normalize(EntityList[TransparentCubeId]->getRotation()));

            Model = TranslationMatrix * RotationMatrix * ScaleMatrix;

            glm::mat4 PrevModel = glm::mat4(1.0);
            ScaleMatrix = glm::scale(glm::mat4(1.0), EntityList[TransparentCubeId]->getPrevScale());
            TranslationMatrix = glm::translate(glm::mat4(1.0), EntityList[TransparentCubeId]->getPrevPosition());
            RotationMatrix;
            RotationMatrix = glm::mat4(1) * glm::toMat4(glm::normalize(EntityList[TransparentCubeId]->getPrevRotation()));

            PrevModel = TranslationMatrix * RotationMatrix * ScaleMatrix;

            glm::vec4 currVector = Model * glm::vec4(1.0f);
            glm::vec4 prevVector = PrevModel * glm::vec4(1.0f);

            glm::vec3 a = (glm::vec3(currVector.x, currVector.y, currVector.z) / currVector.w) * 0.5f + 0.5f;
            glm::vec3 b = (glm::vec3(prevVector.x, prevVector.y, prevVector.z) / prevVector.w) * 0.5f + 0.5f;

            glm::vec3 velocity = glm::abs(a - b);

            if (glm::length(velocity) > 0.0f)
            {
                bottleSpeed += velocity;
                bottleSettleTimer = 2.0;
            }

            bottleSpeed = glm::mix(glm::vec3(0.0), bottleSpeed, bottleSettleTimer / 2.0);
            bottleSettleTimer *= 0.9999f;
        }

        glDisable(GL_BLEND);
        glCullFace(GL_FRONT);
        tShader->PushUniform("bottlePosition", EntityList[TransparentCubeId]->getPosition());
        tShader->PushUniform("alphaBlend", 1.0f);
        tShader->PushUniform("bottleSpeed", bottleSpeed);
        EntityList[TransparentCubeId]->Render();
        glCullFace(GL_BACK);
        // glDisable(GL_DEPTH_TEST);
        glEnable(GL_BLEND);
        tShader->PushUniform("alphaBlend", 0.5f);
        tShader->PushUniform("bottlePosition", EntityList[TransparentCubeId]->getPosition());
        tShader->PushUniform("bottleSpeed", bottleSpeed);
        EntityList[TransparentCubeId]->Render();
        // glEnable(GL_DEPTH_TEST);
        ...
        Shaders["AmbientProbe"]->Use();
        SetUniforms(Shaders["AmbientProbe"], glm::vec3(0, 0, 0), glm::vec3(1.0), glm::quat(-1, 0, -1, 0));
        mInstancedDrawable->Render();
        Shaders["AmbientProbe"]->Free();
