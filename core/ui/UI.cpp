...
glm::vec2 UI::textSize(const std::string &text) {
  ...
  while (*text_data) {
    if (*text_data >= font->first_char &&
        *text_data < font->first_char + font->num_chars) {
      float x = 0, y = 0;
...
void UI::EndWindow() {
  ...
  glm::vec2 white_pixel = glm::vec2(m_Fonts[current_font]->white_pixel.x,
                                    m_Fonts[current_font]->white_pixel.y);
  ...
  while (*text_data) {
    if (*text_data >= font->first_char &&
      ...
      if (*text_data == ' ') {
        ...
        glm::vec4 a = glm::vec4(
            (position) / m_pResolution +
                glm::vec2(q.x0, q.y0 + font->highest_character) / m_pResolution,
            q.s0, q.t0);
        glm::vec4 b = glm::vec4(
            (position) / m_pResolution +
                glm::vec2(q.x1, q.y0 + font->highest_character) / m_pResolution,
            q.s1, q.t0);
        glm::vec4 c = glm::vec4(
            (position) / m_pResolution +
                glm::vec2(q.x0, q.y1 + font->highest_character) / m_pResolution,
            q.s0, q.t1);
        glm::vec4 d = glm::vec4(
            (position) / m_pResolution +
                glm::vec2(q.x1, q.y1 + font->highest_character) / m_pResolution,
            q.s1, q.t1);
...
bool UI::Button(const std::string &text, ButtonFlags flags) {
  ...
  glm::vec4 button_color = (flags.is_tab && flags.is_selected_tab)
...
void UI::Dock(DockLocation location, float thickness) {
  ...
  case DockLocation::BOTTOM:
    CreateRect(glm::vec2(0.0f, m_pResolution.y - thickness),
               glm::vec2(m_pResolution.x, thickness), white_pixel, white_pixel);
    break;
  ...
  case DockLocation::RIGHT:
    CreateRect(glm::vec2(m_pResolution.x - thickness, 0.0f),
               glm::vec2(thickness, m_pResolution.y), white_pixel, white_pixel);
    break;
  }
}
...
bool UI::CheckBox(const std::string &text, bool *state) {
  ...
  CreateRect(position, glm::vec2(switch_width), white_pixel, white_pixel,
...
void UI::HealthBar(const std::string &text, glm::vec2 position, float val,
  ...
  CreateRect(sqr_position, glm::vec2(last_sqr_width, sqr_size.y), white_pixel,
             white_pixel,
...
void UI::Scale(const std::string &text, const std::string &setting_name,
  ...
  float free_space = glm::max(0.0f, currentWindow->setted_size.x -
                                        (estimated_widget_size + name_width));
  ...
  if (fixed_size.x > 0.0) {
    Text(composite_text,
         glm::ceil(
             glm::vec2(widgetStart.x + free_space, position.y) +
  ...
  for (int i = 0; i < glm::clamp(current, 0, max - min); i++) {
    CreateRect(position + glm::vec2((span * i) + m_pStyle.buttonPadding,
                                    (btnSize.y) - 9.0f),
