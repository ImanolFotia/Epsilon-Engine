	...
	public:
		...
		engine::ShaderInfo loadShaders()
		{
			...
			engine::ShaderInfo mainShaderInfo = {
				.stages = {
					{.entryPoint = "main", .shaderCode = vertexCode, .stage = engine::VERTEX},
...
/*
			engine::RenderPassInfo renderPassInfo =
				engine::RenderPassFactory()
					...
					.outputs({{
								  .format = engine::COLOR_RGBA,
								  .blendEnable = true,

								  .clearColor = {0.1f, 0.1f, 0.1f, 0.0f},
								  .isDepthAttachment = false,
								  .isSwapChainAttachment = true,
								  .clearAttachment = true,
							  },
							  {.format = engine::DEPTH_F32_STENCIL_8,
							   .depthStencilValue = {1, 0},
							   .isDepthAttachment = true}})
					.pipelineLayout(uiLayout)
					.pushConstant(72);

			m_pRenderPass = m_pResourceManager->createDefaultRenderPass(renderPassInfo);*/
			// m_pVertices.resize(10000);
			...
			// fread(ttf_buffer, 1, 1 << 20, fopen("./assets/fonts/Roboto-Regular.ttf", "rb"));
			// stbtt_BakeFontBitmap(ttf_buffer, 0, 18.0, temp_bitmap, texture_size, texture_size, 32, 95, cdata);
			/*
			for (int i = 0; i < 95; i++) {
				char c = i;
				float x = 0, y = 0;
			...
			outHeader << "static const unsigned char pixels[] = {\n";
			...
			for (int i = 0; i < 95; i++) {
				outHeader << "{" << m_AlignedQuads[i].x0 << ", " << m_AlignedQuads[i].y0 << ", " << m_AlignedQuads[i].s0 << ", " << m_AlignedQuads[i].t0 << ",";
				outHeader << m_AlignedQuads[i].x1 << ", " << m_AlignedQuads[i].y1 << ", " << m_AlignedQuads[i].s1 << ", " << m_AlignedQuads[i].t1 << "},";
				if (i % 2 == 0) outHeader << "\n";
			}
			outHeader << "};\n";
			outHeader << "}\n";*/
		...
		void AddTexture(const char *path, const char *name)
		{
			int w, h, c = 4;
		...
		void CreateRect(glm::vec2 position, glm::vec2 size, glm::vec2 uv0, glm::vec2 uv1, glm::vec4 color = glm::vec4(1.0, 1.0, 1.0, 0.5));
		...
		void Scale(const std::string &text, int &current, int min, int max, glm::vec2 size);
	};
}