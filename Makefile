...
DX_SDK_INCLUDE := "I:/Epsilon/Epsilon\ Engine/thirdparty/XAudio2/microsoft.xaudio2.redist.1.2.1/build/native/include"
#DX_SDK_LIB := /c/Program\ Files\ \\(x86\\)/Microsoft\ DirectX\ SDK\ \\(June\ 2010\\)/Lib/x64
DX_SDK_LIB := "I:/Epsilon\Epsilon\ Engine/thirdparty/XAudio2/microsoft.xaudio2.redist.1.2.1/build/native/release/lib/x64"
...
$(OBJS_DIR_RELEASE)/%.o: $(SOURCE_DIR)/%.cpp
	-@mkdir -p $(@D)/Release
	$(CXX) $(CPPFLAGS) $(INCLUDE_LIBS) -I$(INCLUDE_DIR) $(CPPFLAGS) -o $@ -c $^  $(LD_FLAGS)

$(BIN)/Release/$(EXEC): $(OBJECTS_RELEASE)
	-@mkdir -p $(@D)/Release
	$(CXX) $(CPPFLAGS) $(RELEASE_FLAGS) $(RES_OBJECTS) -o $@ $^ $(LIBS_DIR) $(LIBS) $(LD_FLAGS) $(CPPFLAGS)

$(BIN)/Debug/$(EXEC): $(OBJECTS_DEBUG)
	-@mkdir -p $(@D)/Debug
	$(CXX) $(CPPFLAGS) $(DEBUG_FLAGS) $(RES_OBJECTS) -o $@ $^ $(LIBS_DIR) $(LIBS) $(LD_FLAGS) $(CPPFLAGS)
