...
Epsilon::Epsilon(const std::string &appName) {
  ...
  m_Context->Window().setWindowTitle(appName.c_str());
}
...
void Epsilon::run() {
  ...
  m_Context->Window().setWindowIcon("./media/icon.png");
  ...
  emscripten_set_main_loop_arg([](void *userData) { mainLoop(); }, nullptr, 0, true);
...
void Epsilon::ShouldClose() { mShouldClose = true; }
...
void Epsilon::setOnCreate(std::function<void(void)> fun) { onCreate = fun; }

void Epsilon::setOnReady(std::function<void(void)> fun) { onReady = fun; }

void Epsilon::setOnRender(std::function<void(void)> fun) { onRender = fun; }

void Epsilon::setOnUpdate(std::function<void(void)> fun) { onUpdate = fun; }

void Epsilon::setOnExit(std::function<void(void)> fun) { onExit = fun; }

void Epsilon::initWindow(int w, int h) { m_Context->Window().init(m_ApplicationName, w, h); }
...
int32_t Epsilon::Frame() { return m_Frame; }
...
EXPORT void CreateContext(const char *name, engine::renderer_type api) { instance.getContext()->Init(name, api); }
...
EXPORT void endScene() { instance.getScene()->EndScene(); }
