...
namespace vk {
    ...
    static VulkanTexture createImage(VulkanData &vkData, VulkanTextureInfo textureInfo) {
        ...
        int mipLevels =
                static_cast<uint32_t>(std::floor(std::log2(std::max(textureInfo.width, textureInfo.height)))) + 1;
    ...
    static void createImageView(const VulkanData &vkData, VulkanTexture &texture, VkImageAspectFlags aspectMask) {
        int mipLevels =
                static_cast<uint32_t>(std::floor(std::log2(std::max(texture.info.width, texture.info.height)))) + 1;
