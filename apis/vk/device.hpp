...
static void createLogicalDevice(VulkanData &vk_data) {
  ...
  queueCreateInfo.resize(MAX_FRAMES_IN_FLIGHT);
  ...
  vkGetDeviceQueue(vk_data.logicalDevice, indices.transferFamily.value(),
                   indices.queueCount > 1 ? 1 : 0, &vk_data.transferQueue);
}
