...
namespace Editor
{
	void Editor::OnCreate()
	{
		...
		m_MenuBar.addCallback("OnExit", [this]()
							  { mShouldClose = true; });
		...
		m_GraphicsHelper->addBindGroup("GrassBindGroup", 3, {
																.bindingInfo = {{.size = sizeof(engine::PBRMaterial) * engine::AssetManager::MAX_MATERIALS, .offset = 0, .binding = 1, .type = engine::UniformBindingType::SHADER_STORAGE, .buffer = "material_buffer"}, {.size = sizeof(glm::mat4) * engine::AssetManager::MAX_TRANSFORMS, .offset = 0, .binding = 5, .type = engine::UniformBindingType::SHADER_STORAGE, .buffer = "transform_buffer"}, {.size = sizeof(engine::ShaderObjectData) * engine::AssetManager::MAX_OBJECTS, .offset = 0, .binding = 6, .type = engine::UniformBindingType::SHADER_STORAGE, .buffer = "object_buffer"}},
																.inputs = {},
																.renderPass = "Forward",
																.name = "GrassBindGroup",
															});
		...
		m_GraphicsHelper->addBindGroup("TAABindGroup0", 0, {
															   .bindingInfo = {{.size = sizeof(Renderpasses::TAAUniformData), .offset = 0, .binding = 1, .type = engine::UniformBindingType::UNIFORM_BUFFER, .buffer = "TAADataBuffer", .name = "TAADataBuffer"}},
															   .inputs = {{.renderPass = "Forward", .index = 0, .bindingPoint = 2}, {.renderPass = "TAARenderPass1", .index = 0, .bindingPoint = 3}, {.renderPass = "Forward", .index = 1, .bindingPoint = 4}, {.renderPass = "Forward", .index = 2, .bindingPoint = 5}},
		...
		m_GraphicsHelper->addBindGroup("TAABindGroup1", 0, {
															   .bindingInfo = {{.size = sizeof(Renderpasses::TAAUniformData), .offset = 0, .binding = 1, .type = engine::UniformBindingType::UNIFORM_BUFFER, .buffer = "TAADataBuffer", .name = "TAADataBuffer"}},
															   .inputs = {{.renderPass = "Forward", .index = 0, .bindingPoint = 2}, {.renderPass = "TAARenderPass0", .index = 0, .bindingPoint = 3}, {.renderPass = "Forward", .index = 1, .bindingPoint = 4}, {.renderPass = "Forward", .index = 2, .bindingPoint = 5}},
		...
		script->data.ManagedPtr = host.assembly.Invoke<void *>(L"CreateEntity", planeNode.get(), "Game.Ground", "Ground");
		...
		m_AssetManager->addTexture("textures/radiance.dds", {.format = engine::TextureFormat::COLOR_RGBA,
															 .wrapMode = engine::CLAMP_TO_EDGE,
															 .filtering = engine::LINEAR});
		m_AssetManager->addTexture("textures/irradiance.dds", {.format = engine::TextureFormat::COLOR_RGBA,
															   .wrapMode = engine::CLAMP_TO_EDGE,
															   .filtering = engine::LINEAR});
