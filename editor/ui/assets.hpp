...
namespace Editor::UI
{
		...
		void draw() override
		{
			...
			if (ImGui::TreeNode("Assets"))
			{
				...
				if (ImGui::BeginTable("3ways", 1, flags, ImVec2(ImGui::GetContentRegionAvail().x, ImGui::GetContentRegionAvail().y * 0.5)))
				{
					...
					{
						DisplayNode({
							.name = assetManager->getImageName(hash),
	...
	private:
		void DisplayNode(AssetProperties asset)
		{
			...
			if (asset.mType == AssetType::Audio)
				icon = ICON_FA_FILE_AUDIO;
			if (asset.mType == AssetType::Script)
				icon = ICON_FA_FILE_CODE;
			if (asset.mType == AssetType::Texture)
				icon = ICON_FA_IMAGES;
			if (asset.mType == AssetType::Shader)
				icon = ICON_FA_GLOBE_EUROPE;
			{
