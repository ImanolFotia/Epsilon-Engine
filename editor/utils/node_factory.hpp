...
namespace Editor::Utils
{
	...
	static void AddScriptNode(ScriptCreationInfo creationInfo, std::shared_ptr<engine::Scene> scene, std::shared_ptr<engine::Node<engine::Scene::SceneEntity>> parent, dotnet::DotnetHost &host)
	{
		...
		script->data.ManagedPtr = host.assembly.Invoke<void *>(L"CreateEntity", parent.get(), creationInfo.className.c_str(), creationInfo.nodeName.c_str());
		...
		args.id = 0;
		args.objectType = dotnet::convert_string(wideName).c_str();
