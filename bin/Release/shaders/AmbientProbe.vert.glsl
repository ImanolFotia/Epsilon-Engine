...
layout(location = 5) in mat4 positions;
...
void main()
{
  ...
  vec4 worldPos = positions * vec4(position, 1.0);
  ...
  vs_out.wFragPos = worldPos.xyz + vec3(1.0);
  Position = Projection * viewPos;
  gl_Position = Projection * viewPos;
  PrevPosition = Projection * PrevView * PrevModel * positions * vec4(position, 1.0);
  ...
  vs_out.Normal = normalize(normal);//normalize(normalMatrix * normalize(normal));
