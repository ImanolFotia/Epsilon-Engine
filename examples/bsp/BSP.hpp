...
public:
  explicit BSP(const std::string &appname) : Epsilon::Epsilon(appname) {
    ...
    Epsilon::onCreate = [this] { onCreate(); };
    Epsilon::onReady = [this] { onReady(); };
    Epsilon::onRender = [this] { onRender(); };
    Epsilon::onExit = [this] { onExit(); };
  }
  ...
  void onCreate() {
    ...
    engine::BindGroupInfo skyInfo = {.bindingInfo = {}, .renderPass = "Default", .name = "SkyMaterial"};
    ...
    m_pMap.pushConstantRef = resourceManager->createPushConstant("ModelMatrix", {.size = sizeof(PushConstant), .data = &m_pMap.pushConstant});
    ...
    Material *materialBufferPtr[vk::MAX_FRAMES_IN_FLIGHT];
    for (int i = 0; i < vk::MAX_FRAMES_IN_FLIGHT; i++) {
      materialBufferPtr[i] = reinterpret_cast<Material *>(resourceManager->mapBuffer(gpuBuffer, i));
    }

    engine::BindGroupInfo bindGroupInfo = {
        .bindingInfo = {{.size = sizeof(Material) * bspMap.numMaterials(), .offset = 0, .binding = 3, .type = en::SHADER_STORAGE, .buffer = "material_buffer"}},
        .inputs =
            {
    ...
    for (auto &[materialName, FaceList] : bspMap.Faces()) {
...

      {
        ...
        std::string mat_name = materialName;
        std::transform(mat_name.begin(), mat_name.end(), mat_name.begin(), [](unsigned char c) { return std::tolower(c); });
        std::string path = "./assets/textures/" + mat_name + ".tga";
        /*
        ...
        if (width == 0 || height == 0 || pixels == nullptr) {
          ...
          for (int i = 0; i < vk::MAX_FRAMES_IN_FLIGHT; i++) {
            materialBufferPtr[i][index].diffuse = 0;
            materialBufferPtr[i][index].normal = -1;
          }
          ...
          for (int i = 0; i < vk::MAX_FRAMES_IN_FLIGHT; i++) {
            materialBufferPtr[i][index].diffuse = id;
            materialBufferPtr[i][index].normal = -1;
          }
...

          {
            ...
            std::string normal_path = "./assets/textures/" + mat_name + "_normal.tga";
  ...
  void onRender() {
    ...
    Epsilon::getContext()->Renderer()->Flush(
        {.renderPassRef = m_pShadowRenderPass, .type = engine::DrawType::INDEXED, .frameBufferIndex = 0, .clean_attachments = true});
    ...
    Epsilon::getContext()->Renderer()->Flush(
        {.renderPassRef = m_pRenderPass, .type = engine::DrawType::INDEXED_INDIRECT, .frameBufferIndex = 0, .clean_attachments = true});
...
private:
  auto setupCamera() -> void {
    auto window_size = getWindowDimensions();
    m_pCameraData.iResolution = glm::vec2(window_size.width, window_size.height);
    ...
    if (m_pUpdateCamera) {
      ...
      m_pCamera->UpdateMatrices(0, window_size.width, window_size.height, false);
    }
  ...
  auto setupShadowCamera() -> void {
    ...
    m_pCameraData.view = glm::lookAt(glm::vec3(23.6365, 25.117, -45.7729), glm::vec3(23.6365, 0.897448, -22.8326), glm::vec3(0.0f, 1.0f, 0.0f));

    m_pCameraData.proj = glm::ortho(-25.0f, 25.0f, -25.0f, 25.0f, -20.f, 100.0f);
    ...
    glm::mat4 ProjectionMatrix =
        glm::perspective(glm::radians(m_pCamera->getFoV()), glm::clamp((float)window_size.height / (float)window_size.width, -10.0f, 10.0f), 0.1f, 25.0f);

    glm::mat4 ViewMatrix = glm::lookAt(m_pCamera->getPosition(), m_pCamera->getPosition() + m_pCamera->getDirection(), glm::vec3(0.0f, 1.0f, 0.0f));
  ...
  auto setupRenderPass() -> void {
    ...
    ShaderInfo mainShaderInfo = {
        .stages = {{.entryPoint = "main", .shaderCode = vertexCode, .stage = VERTEX}, {.entryPoint = "main", .shaderCode = fragmentCode, .stage = FRAGMENT}},
        .usedStages = ShaderModuleStage(VERTEX | FRAGMENT)};

    ShaderInfo shadowShaderInfo = {.stages = {{.entryPoint = "main", .shaderCode = shadowVertexCode, .stage = VERTEX},
                                              {.entryPoint = "main", .shaderCode = shadowFragmentCode, .stage = FRAGMENT}},
                                   .usedStages = ShaderModuleStage(VERTEX | FRAGMENT)};

    ShaderInfo skyShaderInfo = {.stages = {{.entryPoint = "main", .shaderCode = skyVertexCode, .stage = VERTEX},
                                           {.entryPoint = "main", .shaderCode = skyFragmentCode, .stage = FRAGMENT}},
                                .usedStages = ShaderModuleStage(VERTEX | FRAGMENT)};

    VertexLayout vertexLayout = {.descriptors = {{XYZ_FLOAT, offsetof(common::Vertex, position)},
    ...
    PipelineLayout mainLayout = {.shaderInfo = mainShaderInfo,
    ...
    PipelineLayout shadowLayout = {.shaderInfo = shadowShaderInfo,
                                   .vertexLayout = vertexLayout,
    ...
    PipelineLayout skyLayout = {.shaderInfo = skyShaderInfo,
                                .vertexLayout = vertexLayout,
                                .cullMode = CullMode::BACK,
                                .windingMode = WindingMode::CLOCKWISE,
                                .depthWriteEnable = true,
    ...
    RenderPassInfo renderPassInfo =
        RenderPassFactory()
            ...
            .outputs({
                ...
                {.format = DEPTH_F32_STENCIL_8, .depthStencilValue = {1, 0}, .isSampler = false, .isDepthAttachment = true, .clearAttachment = true},
            })
            .pipelineLayout(mainLayout)
            .pipelineLayout(skyLayout)
            .pushConstant(sizeof(PushConstant));
    renderPassInfo.resizeWithSwapChain = true;
    renderPassInfo.numSamples = 1;

    RenderPassInfo shadowRenderPassInfo = RenderPassFactory()
    ...
    renderPassInfo.resizeWithSwapChain = false;
    renderPassInfo.numSamples = 1;
  ...
  auto calculateNormals(std::vector<common::Vertex> &vertices, const std::vector<unsigned int> &indices) -> void {
  ...
  auto calculateNormalsDisplacement(std::vector<common::Vertex> &vertices, const std::vector<unsigned int> &indices) -> void {
